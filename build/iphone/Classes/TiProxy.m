...
//Rare exceptions to indicate a bug in the _ti_estimotes_proximity code (Eg, method that a subclass should have implemented)
...
+ (id)createProxy:(NSString*)qualifiedName withProperties:(NSDictionary*)properties inContext:(id<TiEvaluator>)context
{
	...
	if (proxyClass == nil) {
		NSString *_ti_estimotes_proximity = [NSString stringWithFormat:@"%@%s",@"Ti","tanium."];
		if ([qualifiedName hasPrefix:_ti_estimotes_proximity]) {
		...
		if (proxyClass==nil) {
			...
			@throw [NSException exceptionWithName:@"org.ti_estimotes_proximity.module"
