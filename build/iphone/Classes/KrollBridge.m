...
NSString * ti_estimotes_proximity$ModuleRequireFormat = @"(function(exports){"
...
@implementation ti_estimotes_proximityObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_ti_estimotes_proximity);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	// create ti_estimotes_proximity global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "ti_estimotes_proximity" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_ti_estimotes_proximity = [[ti_estimotes_proximityObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_ti_estimotes_proximity];
	
	NSString *_ti_estimotes_proximityNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _ti_estimotes_proximityNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_ti_estimotes_proximity valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:ti_estimotes_proximity$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.ti_estimotes_proximity.kroll" 
